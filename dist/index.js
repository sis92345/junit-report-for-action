...
const testResultReader_1 = __nccwpck_require__(9371);
const util_1 = __nccwpck_require__(3849);
/**
...
async function run() {
    // Action에서 변수를 받는다.
    const report_path = util_1.UserVariable.of(core.getInput('report_path')).getOrElse('build/junit-test/TEST-*.xml');
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.testResultReaderBasedXml = void 0;
const xmlTestParser_1 = __nccwpck_require__(7591);
const testResultReader = (testParser) => {
    return {
        testParser,
        parseTestResult(globPattern) {
            return testParser.parse(globPattern);
        },
        aggregateTestResult(results) {
            const aggregate = {
                totalCount: 0,
                failed: { count: 0, list: [] },
                passed: { count: 0, list: [] },
...
exports.testResultReaderBasedXml = testResultReader(xmlTestParser_1.xmlTestParser);
...
const util_1 = __nccwpck_require__(3849);
const _private = {
    xmlParser: new fast_xml_parser_1.XMLParser({ ignoreAttributes: false, attributeNamePrefix: "_" }),
    validateXml: (resultRawJson) => {
...
exports.xmlTestParser = {
    parse(globPattern) {
        const resultFileStr = (0, glob_1.globSync)(globPattern)
            .map(path => {
            try {
                return (0, fs_1.readFileSync)(path, "utf-8");
            }
            catch (e) {
                const nodeJsException = e;
        ...
        return resultFileStr.map(fileStr => util_1.UserVariable.of(_private.xmlParser.parse(fileStr))
            .getOrElse({
            ...
            .map((result) => {
            const status = result.failure == undefined && result.skipped == undefined
                ? "passed"
...
class UserVariable {
    ...
    map(fn) {
        return this.isJust() ? UserVariable.of(fn(this.value)) : UserVariable.nothing();
    }
    flatMap(fn) {
        return this.isJust() ? fn(this.value) : UserVariable.nothing();
    }
    getOrElse(defaultValue) {
        return this.isJust() ? this.value : defaultValue;
    }
}
exports.UserVariable = UserVariable;
